...
void base::active_control(void)
{
	...
	if(!keystates['a'] && !keystates['d'] && !up)	//idle
		//~ dBodyAddForce(body,-pointrelvel[0]/1.5,0,-pointrelvel[2]); //for circle only! //remove third arg
		dBodyAddForce(body,-pointrelvel[0]/1.5,0,0); //for circle only! //remove third arg
	...
	if(keystates['q']) printf("X:%.4f Y:%.4f Z:%.4f \n", odepos[0],odepos[1],odepos[2]);
	if(keystates['z']) dBodySetPosition(body,odepos[0],odepos[1],0);
	hack_2d();
...
void base::passive_control(void)
{
	...
	if(!up)	//idle
		dBodyAddForce(body,-pointrelvel[1]/1.5,0,0); //for circle only!
