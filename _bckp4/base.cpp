...
void base::square_render (void)
{
	...
	if(keystates['t']) printf("UP:%i LAST:%i\n", up, last);
	//~ if(!keystates['a'] && !keystates['d'] && !up)
	if(!keystates['a'] && !keystates['d'] && !up)
	{//idle
	...
	if(pointrelvel[0]<=1 && pointrelvel[0]>=-1 && !up) //stand animation
		...
		//~ if(last==2 || last==3 || last==4 || last==5) last-=2; //work
	...
	//---------------------------------
	//~ printf("%.4f\n",rot);
	if(!up) glBindTexture( GL_TEXTURE_2D, texture[last].texture );
	else glBindTexture( GL_TEXTURE_2D, texture[last].texture );
...
short base::check_state(void)
{
	...
	if (pointrelvel[1] > -1 && pointrelvel[1] < 1)
	{
		...
		if (stand>=5) Tup=0, Dup=0, c_tmp=0;		//выключаем дж на 3емле
